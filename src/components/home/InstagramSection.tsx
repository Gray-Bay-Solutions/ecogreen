  ...
  for (const tour of tours as Tour[]) {
    for (const src of tour.gallery) {
      if (!seen.has(src)) {
        seen.add(src);
        images.push(src);
        if (images.length >= PREVIEW_COUNT) return images;
      }
    }
  }
      ...
      <div className="container-custom">
        <div className="text-center mb-10">
          ...
          <p className="text-gray-600 max-w-2xl mx-auto">
            See real tour moments, Nosara scenery, and the latest adventures from Eco Green Nosara.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 mb-10">
          {previewImages.map((src, index) => (
            <a
              ...
              aria-label={`View tour photo ${index + 1} on Instagram`}
            >
              <Image
                src={src}
                alt={`Eco Green Nosara tour photo ${index + 1}`}
                fill
        ...
        <div className="text-center">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-3 bg-gradient-to-r from-[#833AB4] via-[#FD1D1D] to-[#F77737] text-white font-semibold py-3 px-6 rounded-lg hover:opacity-90 transition-opacity"
          >
